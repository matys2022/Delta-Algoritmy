...
//                System.out.println("X: " + pixelX + " Y: " + pixelY);
        ...
        // Now it compares first, then attaches the boolean (true/false) to the string
...
//        System.out.println("Has overflown? : " + (index >= element.getChildren().size()));
        while((element.getChildren() != null && (!element.getChildren().isEmpty()) && index < element.getChildren().size())){
            rasterize(element.getChildren().get(index), index);
            index++;
        }
